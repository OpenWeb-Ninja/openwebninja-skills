...
async function main() {
    ...
    const meta = loadMeta('chatgpt');
    const host = meta.rapidapi_host || 'chatgpt194.p.rapidapi.com';
    const format = args.format || 'json';
    const dryRun = !!args.dryRun;

    const ts = new Date().toISOString().replace(/[:.]/g, '-').slice(0, 19);
    const outputPath = args.output || path.join('output', `chatgpt_${ts}.${format}`);
    ...
    console.error('Sending message to ChatGPT...');
    ...
    writeOutput([record], outputPath, format, { api: 'chatgpt', endpoint: '/chat', totalCalls: 1 });
}
