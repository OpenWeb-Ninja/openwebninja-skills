...
Send a message to Google Gemini and save the response.
...
async function main() {
    ...
    const meta = loadMeta('gemini');
    const host = meta.rapidapi_host || 'gemini24.p.rapidapi.com';
    const format = args.format || 'json';
    const dryRun = !!args.dryRun;

    const ts = new Date().toISOString().replace(/[:.]/g, '-').slice(0, 19);
    const outputPath = args.output || path.join('output', `gemini_${ts}.${format}`);
    ...
    console.error('Sending message to Gemini...');
    const body = { message: prompt };
    if (args.conversationId) body.conversation_id = args.conversationId;

    const data = await apiCall(host, '/chat', {}, apiKey, 'POST', body);
    ...
    writeOutput([record], outputPath, format, { api: 'gemini', endpoint: '/chat', totalCalls: 1 });
}
